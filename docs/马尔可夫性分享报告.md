...
### 例子1：ReActAgent的内部状态管理（不符合马尔可夫性）
...
1. **状态污染**：
```typescript
// 同一个blackboard可能被多个ReActAgent实例使用
// 状态相互干扰
...
2. **调试困难**：
```typescript
// 问题出现时，难以确定状态是如何变成这样的
console.log(blackboard.get('react_iteration')); // 3
// 是正常执行到第3次，还是异常情况？
```
...
// 测试前需要清理所有相关状态
beforeEach(() => {
    blackboard.set('react_iteration', 0);
    blackboard.set('react_thoughts', []);
    blackboard.set('react_observations', []);
    // 容易遗漏某些状态
});
```
...
// AI重启后，所有推理历史丢失
// 无法从中断处恢复
blackboard.set('react_iteration', 0); // 从头开始
```
