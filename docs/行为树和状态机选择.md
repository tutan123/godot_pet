...
**强烈建议：Godot 本地用【BlendTree】，JS 服务端用【行为树】。**

由于我们采用了**马尔科夫性 (Markov Property)** 的设计理念，决策权完全留在 AI 手中，现在的权责划分如下：

| 特性 | 本地 BlendTree (Godot) | 服务端行为树 (BehaviorTree.js) |
| :--- | :--- | :--- |
| **马尔科夫性** | **表现层转换**：根据当前参数（输入）驱动到目标姿态。 | **决策层转移**：根据当前黑板状态（Sensor）转移到下一个意图（Actuator）。 |
| **职责** | 动画混合、物理即时响应。 | 意图理解、主动干预、逻辑锁定。 |
| **隔离模式** | **物理输入优先**：键盘输入立即覆盖表现。 | **输入输出隔离**：感知状态（Sensor）与 下达指令（Actuator）分离。 |

#### 马尔科夫性在项目中的体现：
1.  **输入即传感器**：Godot 本地物理状态（isMovingLocally, isJumpPressed）作为黑板的 `Sensor` 输入给 AI。
2.  **AI 无记忆状态转移**：行为树每一帧根据这些传感器输入，重新评估优先级，并决定当前的意图（Actuator）。
3.  **不干预决策**：当 AI 检测到用户正在操作时，它会主动决策进入“静默状态”，从而让出控制权，而不是被动地被拦截。

### 4. 关于黑板系统与数据同步 (案例一：输入输出双变量模式)

我们采用了类似 `pointerPosition` (输入) 与 `bt_output_position` (输出) 的双变量隔离模式：
*   **Sensor Key (输入)**：`isMovingLocally` (客户端物理情况)。
*   **Actuator Key (输出)**：`bt_output_action` (AI 主动干预意图)。

这种模式确保了：当你自己控制 WASD 时，AI 观察到了你的行为，它会决策“保持静默”，因此不会产生回声指令，彻底消除了网络冲突带来的延迟。
...
graph TD
    ...
    subgraph "Godot 客户端 (本地身体 - BlendTree)"
        G[WebSocketClient] -->|接收指令| H[PetController]
        H -->|物理响应/拖拽控制| I{Markov Decision}
        I -->|高优先级: 用户物理交互| J[AnimationTree: 参数即时驱动]
        I -->|低优先级: AI 意图指令| J
        J -->|驱动| K[骨骼动画: Idle/Walk/Run]
        J -->|叠加| L[程序化动画: Spin/Bounce/Fly]
        H -->|传感器上报| M[Sensor Sync: isMovingLocally/Pos]
    end
...
1.  **马尔科夫感知 (Markovian Sensing)**：JS 端的行为树不再保留“历史命令”，而是每一帧观察黑板上的 `Sensor` 数据（如 `isMovingLocally`）。
2.  **输入输出隔离**：我们通过双变量模式（感知变量 vs 指令变量）解决了网络回声问题。用户自己 WASD 时，AI 观察并决策“不发出干扰指令”。
3.  **双层执行**：Godot 的 `AnimationTree` (BlendTree) 保证了参数到动作的即时转换，而 JS 行为树则负责高层逻辑转移。
