...
```mermaid
graph TD
    subgraph Server["服务端 (JS/Node.js)"]
        LLM[LLM API]
        FunctionExec[FunctionExecNode]
        PendingArray[pendingActions Array]
        ExecuteSeq[ExecuteActionSequence]
        LockState[actionLock<br/>actionLockPriority<br/>actionLockToken]
        OutputState[bt_output_action<br/>bt_output_action_duration<br/>bt_output_action_priority<br/>bt_output_action_interruptible]
        BTServer[BTServer.sendBTOutputs]
        WSOutput[WebSocket: bt_output<br/>actionState: {...}]
        
        LLM --> FunctionExec
        FunctionExec --> PendingArray
        PendingArray --> ExecuteSeq
        ExecuteSeq --> LockState
        LockState --> OutputState
        OutputState --> BTServer
        BTServer --> WSOutput
    end
    
    WSOutput -->|WebSocket 协议| WSInput
    
    subgraph Client["客户端 (Godot/GDScript)"]
        WSInput[WebSocketClient<br/>_handle_json_message]
        Controller[pet_controller<br/>_on_ws_message bt_output]
        ApplyState[_apply_action_state]
        PriorityCheck[优先级检查<br/>中断判断]
        CurrentState[current_action_state = {...}]
        SwitchAnim[_switch_anim]
        AnimTree[AnimationTree.play<br/>或程序化动画]
        
        WSInput --> Controller
        Controller --> ApplyState
        ApplyState --> PriorityCheck
        PriorityCheck --> CurrentState
        CurrentState --> SwitchAnim
        SwitchAnim --> AnimTree
    end
```
...
当前系统采用**"本地物理闭环 + 服务端 AI 决策 + 状态声明式通信"**的架构。客户端和服务端通过**黑板系统 (Blackboard)** 进行状态同步，实现输入输出隔离的马尔可夫决策模型。

```mermaid
graph TB
    subgraph Client["Godot 客户端 (表现层)"]
        Input[用户输入<br/>WASD/Space/鼠标]
        Physics[物理引擎<br/>60fps]
        BlendTree[AnimationTree<br/>BlendTree]
        StateSync[状态同步<br/>state_sync]
    end
    
    subgraph Comm["通信层 (WebSocket)"]
        WS1[state_sync<br/>客户端→服务端]
        WS2[bt_output<br/>服务端→客户端]
    end
    ...
    subgraph Server["JS 服务端 (决策层)"]
        ...
        LLMNode[LLM 决策节点]
        Output[动作输出<br/>bt_output_action]
    end
...
### 9.5 BlendTree 与行为树的关系

#### 9.5.1 设计理念

**BlendTree (客户端)** 和 **BehaviorTree (服务端)** 是两个不同层次的系统：

- **BlendTree**：负责**表现层的参数混合**，实现动画的无缝过渡和多维度叠加
- **BehaviorTree**：负责**决策层的意图生成**，决定应该执行什么动作

```mermaid
graph LR
    subgraph Decision["决策层 (服务端 BehaviorTree)"]
        BT[行为树节点<br/>ExecuteActionSequence<br/>PlayAnimationAction]
        Intent[意图输出<br/>bt_output_action: DANCE]
    end
    
    subgraph Communication["通信层"]
        WS[WebSocket<br/>bt_output message]
    end
    
    subgraph Performance["表现层 (客户端 BlendTree)"]
        Param[混合参数<br/>blend_position: 0.0-1.0]
        Blend[AnimationTree<br/>BlendTree]
        Anim[动画播放<br/>Idle/Walk/Run/Dance]
    end
...
sequenceDiagram
    ...
    participant Server as JS服务端
    participant BT as BehaviorTree
    participant Blend as BlendTree
    
    User->>Client: 输入 "跳个舞"
    Client->>Server: user_input message
    Server->>BT: LLM 调用，解析意图
    BT->>BT: ExecuteActionSequence
    BT->>Server: bt_output_action: DANCE
    Server->>Client: bt_output {actionState: {name: "DANCE"}}
    Client->>Blend: _apply_action_state()
    Client->>Blend: 设置 blend_position
    Blend->>Blend: 平滑过渡到 DANCE 动画
    Blend->>User: 角色开始跳舞
```

#### 9.5.3 关键区别

| 维度 | BlendTree (客户端) | BehaviorTree (服务端) |
|:-----|:------------------|:---------------------|
...
```mermaid
graph TB
    subgraph ClientV1["Godot 客户端（本地闭环）"]
        WASD[WASD 移动 ✓]
        Jump[跳跃 ✓]
        Drag[拖拽 ✓]
        Physics1[基础物理 ✓]
    ...
    subgraph ServerV1["JS 服务端（可视化 + 高级决策）"]
        Visual[行为树可视化 ✓]
        LLM[LLM 动作序列 ✓]
        Data[数据收集 ✓]
    end
    
    ClientV1 -->|state_sync 1s| ServerV1
    
    style ClientV1 fill:#e1f5ff
    style ServerV1 fill:#fff4e1
```
...
```mermaid
graph TB
    subgraph ClientV2["Godot 客户端（表现层）"]
        Anim[所有动画播放]
        Phys[物理引擎]
        Input[输入响应]
    end
    
    subgraph ServerV2["JS 服务端（决策层）"]
        Decision[完整行为树决策]
        Path[路径规划]
        Emotion[能量/情绪系统]
        Sync[多客户端同步]
    ...
    ClientV2 <-->|指令/状态| ServerV2
    
    style ClientV2 fill:#e1f5ff
    style ServerV2 fill:#fff4e1
```
...
**Blackboard (黑板)** 是客户端和服务端之间的**共享内存**，通过 WebSocket 消息进行同步。它遵循**马尔可夫决策模型**，将数据划分为传感器 (Sensors) 和执行器 (Actuators)。

```mermaid
graph LR
    subgraph Sensors["传感器数据 (Sensors)<br/>客户端 → 服务端"]
        S1[isMovingLocally<br/>是否正在移动]
        S2[isJumpPressed<br/>是否按下跳跃]
        S3[isDragging<br/>是否拖拽中]
        S4[penguinPosition<br/>位置坐标]
        S5[velocity<br/>速度向量]
        S6[isOnFloor<br/>是否在地面]
        S7[lastCollision<br/>碰撞信息]
    end
    
    subgraph Blackboard["Blackboard 黑板"]
        BB[(共享状态存储)]
    end
    
    subgraph Actuators["执行器数据 (Actuators)<br/>服务端 → 客户端"]
        A1[bt_output_action<br/>动作名称]
        A2[bt_output_action_duration<br/>持续时间]
        A3[bt_output_action_priority<br/>优先级]
        A4[bt_output_action_interruptible<br/>是否可中断]
        A5[bt_output_position<br/>目标位置]
    end
    
    Sensors -->|state_sync| Blackboard
    Blackboard -->|bt_output| Actuators
    
    style Sensors fill:#c8e6c9
    style Actuators fill:#ffccbc
    style Blackboard fill:#fff9c4
```
...
| 变量名 | 类型 | 更新频率 | 说明 | 来源 |
|:------|:-----|:--------|:-----|:-----|
| `isMovingLocally` | Boolean | 实时 (state_sync) | 客户端是否正在由 WASD 手动控制 | Godot `_physics_process` |
| `isJumpPressed` | Boolean | 实时 (state_sync) | 用户是否正在按跳跃键 | Godot 输入检测 |
| `isDragging` | Boolean | 按需 (drag_start/end) | 角色是否处于被拖拽状态 | Godot 鼠标事件 |
| `penguinPosition` | Array[3] | 1秒 | 角色的 3D 坐标 [x, y, z] | Godot `global_position` |
| `velocity` | Array[3] | 实时 (state_sync) | 当前速度向量 | Godot `velocity` |
| `isOnFloor` | Boolean | 实时 (state_sync) | 是否在地面上 | Godot `is_on_floor()` |
| `lastCollision` | Object | 碰撞时 | 碰撞信息 {collider_name, position, normal, timestamp} | Godot 碰撞检测 |
| `isClicked` | Boolean | 按需 (click) | 鼠标点击事件 | Godot 输入事件 |

**执行器变量 (服务端 → 客户端)**

| 变量名 | 类型 | 更新频率 | 说明 | 来源 |
|:------|:-----|:--------|:-----|:-----|
| `bt_output_action` | String | 状态变化时 | AI 产生的动作名称 (如 "DANCE", "WAVE") | BehaviorTree 节点 |
| `bt_output_action_duration` | Number | 状态变化时 | 动作持续时间（毫秒） | BehaviorTree 节点 |
| `bt_output_action_priority` | Number | 状态变化时 | 动作优先级（10-100+） | BehaviorTree 节点 |
| `bt_output_action_interruptible` | Boolean | 状态变化时 | 是否可被更高优先级中断 | BehaviorTree 节点 |
| `bt_output_position` | Array[3] | 寻路时 | AI 要求的移动目标位置 | MoveToTargetAction |
| `pendingActions` | Array[String] | LLM调用后 | 待执行的动作序列 | FunctionExecNode |

**内部状态变量 (服务端内部)**

| 变量名 | 类型 | 说明 |
|:------|:-----|:-----|
| `actionLock` | Boolean | 是否被动作锁定 |
| `actionLockToken` | String | 锁定者的节点ID |
| `actionLockPriority` | Number | 当前锁定的优先级 |
| `energy` | Number | 能量值 (0-100) |
| `boredom` | Number | 无聊度 (0-100) |
...
**客户端处理**：
- 接收 `actionState`
- 调用 `_apply_action_state()` 进行优先级检查
- 如果通过，更新 `current_action_state`
- 切换动画或执行程序化动画
...
sequenceDiagram
    ...
    Note over Client,Blend: 用户按下 W 键
    Client->>Client: _physics_process()<br/>检测输入，开始移动
    ...
    BT->>BB: 不写入 bt_output_action (让路)
    Note over BT: AI 决策：保持静默
    
    Note over Client,Blend: 用户输入 "跳个舞"
    Client->>WS: user_input {text: "跳个舞"}
    WS->>Server: 接收消息
    Server->>BT: LLM 调用
    BT->>BB: set('pendingActions', ['DANCE'])
    BT->>BB: set('bt_output_action', 'DANCE')
    Server->>WS: bt_output {actionState: {name: "DANCE"}}
    WS->>Client: 接收消息
    Client->>Client: _apply_action_state()
    Client->>Blend: 设置 blend_position 切换动画
    Blend->>Blend: 平滑过渡到 DANCE
```
